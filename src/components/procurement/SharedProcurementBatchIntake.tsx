...
type SharedProcurementBatchIntakeProps = {
  ...
  documentLinkHrefBuilder?: (documentId: string) => string | null;
};
              ...
              <tr key={item.id} className="border-t border-slate-800/60 align-top">
                ...
                <td className="px-4 py-3">
                  <div className="space-y-1">
                    {item.documentId ? (
                      (() => {
                        const detailHref = documentLinkHrefBuilder?.(item.documentId) ?? null;
                        if (!detailHref) {
                          return <span className="text-slate-500">{item.documentId.slice(0, 8)}…</span>;
                        }
                        return (
                          <Link href={detailHref} className="text-sky-300 underline">
