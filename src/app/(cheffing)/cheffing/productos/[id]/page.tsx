...
export default async function CheffingProductoDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: product, error: productError } = await supabase
    ...
    .select(
      'id, name, purchase_unit_code, purchase_pack_qty, purchase_price, waste_pct, categories, reference, stock_unit_code, stock_qty, min_stock_qty, max_stock_qty, allergen_codes, indicator_codes, created_at, updated_at',
    )
    .eq('id', params.id)
    .maybeSingle();
