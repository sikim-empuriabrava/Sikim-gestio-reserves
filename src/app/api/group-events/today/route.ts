...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('group_events')
      .select(
        `id, name, event_date, entry_time, adults, children, total_pax, event_mode, party_room_id, party_room:rooms!group_events_party_room_id_fkey(name), status, menu_text, second_course_type, seconds_confirmed, allergens_and_diets, extras, setup_notes, has_private_dining_room, has_private_party`
      )
      .eq('event_date', today)
