...
export async function GET() {
  ...
  const [menusResult, cardsResult] = await Promise.all([
    supabase
      .from('cheffing_menus')
  ...
  if (menusResult.error) {
    const response = NextResponse.json(
      { error: menusResult.error.message ?? 'Unknown error loading menus' },
  ...
  if (cardsResult.error) {
    const response = NextResponse.json(
      { error: cardsResult.error.message ?? 'Unknown error loading cards' },
      { status: 500, headers: noStoreHeaders },
    );
    mergeResponseCookies(supabaseResponse, response);
    return response;
  ...
  const activeMenus = (menusResult.data ?? []) as MenuCatalogRow[];
  const activeCards = (cardsResult.data ?? []) as CardCatalogRow[];
  ...
  const cardOfferings = activeCards.map((card) => ({
  ...
  const response = NextResponse.json(
    {
      offerings: [...menuOfferings, ...cardOfferings].sort((a, b) => a.display_name.localeCompare(b.display_name, 'es')),
    },
