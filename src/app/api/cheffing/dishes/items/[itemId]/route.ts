...
export async function PATCH(req: NextRequest, { params }: { params: { itemId: string } }) {
  ...
  const hasSubrecipeKey = body && Object.prototype.hasOwnProperty.call(body, 'subrecipe_id');
  ...
  if (hasIngredientKey || hasSubrecipeKey) {
    ...
    const subrecipeId = typeof body?.subrecipe_id === 'string' ? body.subrecipe_id.trim() : null;

    if ((!ingredientId && !subrecipeId) || (ingredientId && subrecipeId)) {
    ...
    updates.subrecipe_id = subrecipeId;
  }
  ...
  const { error } = await supabase.from('cheffing_dish_items').update(updates).eq('id', params.itemId);
