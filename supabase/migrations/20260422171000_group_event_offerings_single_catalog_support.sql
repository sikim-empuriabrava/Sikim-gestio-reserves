...
begin
  ...
  loop
    ...
    if v_kind = 'cheffing_menu' then
      ...
      if v_offering_id_text is null then
        raise exception 'offeringAssignments[%].offeringId/menuId is required', v_idx - 1;
      end if;
    else
    ...
    insert into _tmp_offering_assignments (offering_kind, cheffing_menu_id, cheffing_card_id, assigned_pax, sort_order, notes)
    values (
      v_kind,
  ...
  select
    ...
    t.sort_order,
    case
      when t.offering_kind = 'cheffing_menu' then jsonb_build_object(
  ...
  left join public.cheffing_menus m on m.id = t.cheffing_menu_id
  left join public.cheffing_cards c on c.id = t.cheffing_card_id
  order by t.sort_order, t.cheffing_menu_id, t.cheffing_card_id;
